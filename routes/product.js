	...
	user.findOne({ _id: db.ObjectID(userId) }, function (err, user_data) {
		if (err) res.status(500).send({ status: 0, message: err.message });
			...
			if (user_data) {
				...
				products.update({_id : db.ObjectID(product_id)}, {$set : query}, function(err, product_validation){
					if(err) res.status(500).send({ status: 0, message: err.message });
					else{
						if(product_validation){

							res.send({ status: 1, message: 'Producto actualizado correctamente' });
	...
	user.findOne({ _id: db.ObjectID(userId) }, function (err, user_data) {
		if (err) res.status(500).send({ status: 0, message: err.message });
			...
			if (user_data) {
				...
				products.remove({_id : db.ObjectID(product_id)}, function(err, product_data){
					if(err) res.status(500).send({ status: 0, message: err.message });
					else{
						if(product_data){
