(function(){
	...
	Dropzone.options.uploadFile = {
		...
		init: function() {
			...
			var secondDzMessageSpan 		= $('#load_image_span_categorie')
			var thirdDzMessageSpan 			= $('#load_image_span_subcategorie')
			var fourthDzMessageSpan 		= $('#load_image_span_content')
			var fiveDzMessageSpan			= $('#load_image_brands_span')
		...
		// 	});
			...
			this.on('uploadprogress', function(file, progress){
				...
				secondDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...' + progress.toFixed(2)+'%');
				thirdDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...' + progress.toFixed(2)+'%');
				fourthDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...' + progress.toFixed(2)+'%');
				fiveDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...' + progress.toFixed(2)+'%');
			});
			...
			this.on("sending", function(file, xhr, formData) {
				...
				secondDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...');
				thirdDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...');
				fourthDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...');
				fiveDzMessageSpan.html('Subiendo <b>'+ file.name+'</b>, pulsa para cancelar...');
			...
			this.on('success', function(file, res){
				...
				if (res.secure_url) {
					...
					secondDzMessageSpan.html('Archivo subido: ' + file.name);
					thirdDzMessageSpan.html('Archivo subido: ' + file.name);
					fourthDzMessageSpan.html('Archivo subido: ' + file.name);
					fiveDzMessageSpan.html('Archivo subido: ' + file.name);
					...
					$('#input_categorie_image_url').val(url)
					$('#input_subcategorie_image_url').val(url)
					$('#input_content_image_url').val(url)
					...
					$('#images_brands_upload').slideDown()
					$('#images_brands_upload').append(
												'<div class="col-md-3" style="'+
													'background-image: url('+url_thumb+');'+
													
													'background-repeat: no-repeat; height: 99px;">'+
														'<a href="javascript:;" class="btn_principalimage" style="margin-right: 10px; margin-top:1em;"></a>'+
														'<a href="javascript:;" id="btn_deleteimage">'+
															'<span class="badge badge-danger">'+
																'<i class="fa fa-close"></i>'+
															'</span>'+
