(function(){
	...
	$('body').on('click', '#btn_update_content', function(){
		...
		if(content_description == 'MARCAS'){
			$('#content_without_brands').fadeOut();
			$('#content_with_brands').fadeIn();
		}else{
			$('#content_without_brands').fadeIn();
			$('#content_with_brands').fadeOut();
		}
	...
	$('body').on('click', '#btn_update_content_body', function(){
		...
		$('#images_brands_upload').find('.col-md-3').each(function(){
		...
		$.ajax({
			...
			type: 'post',
			contentType: 'application/json',
			data: JSON.stringify({ content_description : content_description, content_body : content_body, subtitle : subtitle, image : image }),
			success: function (data) {
				console.log(data)

				$('#summernote').summernote('code', '');
				$('#input_content_subtitle').val('');
				$('#images_brands_upload').empty()
	...
	$('body').on('click', '#btn_view_product_variables', function(){
			...
			success: function (data) {
				...
				if(variables.length){
					for(var i=0; i<variables.length; i++){
						$(tr).after('<tr>'+
														'<td>'+(variables.length-i)+'</td>'+
														'<td>'+variables[i].variable_code+'</td>'+
														'<td>'+variables[i].variable_name+'</td>'+
														'<td>'+Number(variables[i].variable_price).toFixed(2)+'</td>'+
														'<td>'+variables[i].variable_stock+'</td>'+
														'<td>'+
														'</td>'+
													'</tr>')
						console.log('dtat', i, variables.length)
						if((i+1)==variables.length){
							$(tr).after('<tr>'+
														'<th style="font-weight: 500;">#</th>'+
														'<th style="font-weight: 500;">Codigo</th>'+
														'<th style="font-weight: 500;">Presentación</th>'+
														'<th style="font-weight: 500;">Precio</th>'+
					...
					if(data.data.length == 0){
						...
						for(var i=0; i<products_data.length; i++){
							...
							$('#tbody_products').append('<tr>'+
									'<td>'+'<a id="btn_view_product_variables" value="'+products_data[i]._id+'" href="javascript:;" class="btn btn-default btn-xs" style="padding: 1px 3px 1px 8px;"><i class="fa fa-info-circle"></i></a>'+'</td>'+
									'<td>'+products_data[i].categorie_id.name+'</td>'+
									'<td>'+subcategorie_name+'</td>'+
									'<td>'+product_name+'</td>'+
									'<td>'+products_data[i].stock+'</td>'+
									'<td>'+
										'<a id="btn_update_product" value="'+products_data[i]._id+'" href="javascript:;" class="btn c-theme-btn btn-xs" style="padding: 1px 3px 1px 8px;"><i class="fa fa-edit"></i></a>'+
										'<a id="btn_remove_product" value="'+products_data[i]._id+'" href="javascript:;" class="btn c-theme-btn btn-xs" style="padding: 1px 3px 1px 8px;"><i class="fa fa-trash"></i></a>'+
									'</td>'+
								'</tr>')
							//_.truncate(detail_purchaseorder[i].paymentconditionsdescription, {'length': 50, 'omission': ' [...]'});
						}
	...
	$('body').on('click', '#btn_remove_product', function(){
		...
		var confirmation = confirm('¿Está seguro se realizar esta acción?');
	...
	function cancel_categorie_form(){
		...
		$('#input_categorie_image_url').val('')
		$('#input_subcategorie_image_url').val('')
		$('#load_image_span_categorie').text('')
		$('#load_image_span_subcategorie').text('')
		$('#load_image_span_content').text('')
		$('#input_content_image_url').text('')
		$('#load_image_brands_span').text('')
		$('#load_image_brands_span').text('')
	...
	$('body').on('click', '#btn_add_product', function(){
		...
		if(name && categorie_id && content && image.length != 0){

				stock = 0;

				$('#tbody_variable').find('tr').each(function(){

					var variable_code 		= $(this).find('td:nth-child(2) input').val();
					var variable_name 		= $(this).find('td:nth-child(3) input').val();
					var variable_stock 		= $(this).find('td:nth-child(4) input').val();
					var variable_price 		= $(this).find('td:nth-child(5) input').val();

					stock = Number(variable_stock) + stock

					variables.push({variable_code : variable_code, variable_name : variable_name, variable_stock : variable_stock, variable_price : variable_price})
				})
	...
	$('body').on('click', '#btn_update_product_info', function(){
		...
		var product_id 					= $('#input_product_selected').val();
		var name 						= $('#input_product_name').val();
		var categorie_id 				= $('#select_categorie_product').val();
		var subcategorie_id 			= $('#select_subcategorie_product').val();
		var content 					= $('#summernote03').summernote('code');
		...
		if(validation == 'enable'){
			...
			$.ajax({
				...
				success: function (data) {
					...
					$('.alert.alert-success.products02').fadeIn();
	...
	$('body').on('click', '#btn_update_product', function(){
		...
		$('#btn_add_product').prop('id', 'btn_update_product_info')
		...
		$.ajax({
			...
			success: function (data) {
				...
				var _id 			= data.data._id;
				var categorie_id 	= data.data.categorie_id;
				var content 		= data.data.content;
				var image 			= data.data.image;
				var name 			= data.data.name;
				var stock 			= data.data.stock;
				var subcategorie_id = data.data.subcategorie_id;
				var variables 		= data.data.variables;
				...
				if(variables.length){
					for(var i=0; i<variables.length; i++){
						$('#tbody_variable').append('<tr>'+
														'<td>1</td>'+
														'<td><input type="text" class="form-control c-square c-theme" value="'+variables[i].variable_code+'"></td>'+
														'<td><input type="text" class="form-control c-square c-theme" value="'+variables[i].variable_name+'"></td>'+
														'<td><input type="text" class="form-control c-square c-theme" value="'+variables[i].variable_stock+'"></td>'+
														'<td><input type="text" class="form-control c-square c-theme" value="'+variables[i].variable_price+'"></td>'+
														'<td>'+
															'<a id="btn_remove_variable" href="javascript:;" class="btn c-theme-btn btn-xs" style="padding: 1px 3px 1px 8px;"><i class="fa fa-trash"></i></a>'+
														'</td>'+
													'</tr>')
					}
				}
				...
				if(image.length){
					...
					$('#images_upload').fadeIn();

					for(var i=0; i<image.length; i++){

						var document_id 		= image[i].document_id;
						var document_type 		= image[i].document_type;
						var document_url 		= image[i].document_url;
						var document_url_thumb 	= image[i].document_url_thumb;
						var principal 			= image[i].principal;

						if(principal == 'principal'){
							$('#images_upload').append('<div class="col-md-3" style="'+
															'background-image: url('+document_url_thumb+');'+
															
															'background-repeat: no-repeat; height: 99px;">'+
																'<a href="javascript:;" class="btn_principalimage" style="margin-right: 10px; margin-top:1em;"><span class="badge badge-danger" style="background-color: rgb(246, 249, 46);"><i class="fa fa-star"></i></span></a>'+
																'<a href="javascript:;" id="btn_deleteimage">'+
																	'<span class="badge badge-danger">'+
																		'<i class="fa fa-close"></i>'+
																	'</span>'+
																'</a>'+
																'<input type="hidden" id="document_type" value="'+document_type+'" />'+ 
																'<input type="hidden" id="document_id" value="'+document_id+'" />'+ 
																'<input type="hidden" id="document_url" value="'+document_url+'" />'+ 
																'<input type="hidden" id="document_url_thumb" value="'+document_url_thumb+'" />'+ 
														'</div>' );
						}else{
							$('#images_upload').append('<div class="col-md-3" style="'+
															'background-image: url('+document_url_thumb+');'+
				...
				$('#input_product_name').val(name)
				$('#summernote03').summernote('code', content);
				$('#select_categorie_product option[value="'+categorie_id+'"]').prop('selected', true);
				...
				$.ajax({
					...
					success: function (data) {
						...
						$('#select_subcategorie_product option[value="'+subcategorie_id+'"]').prop('selected', true);
