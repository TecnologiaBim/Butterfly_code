(function(){
	...
	function get_subcategorie_by_id(){

		var subcategorie_id = window.location.pathname.split('/')[2];
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('#subcategorie_name').text('Sub Categoría - ' + name)
				$('#subcategorie_content').html(content + '<br><br><br>')
	...
	function load_products_by_subcategorie_id(page){

		var subcategorie_id = window.location.pathname.split('/')[2];

		$.ajax({
			url: '/load_products_by_subcategorie_id',
			type: 'post',
			contentType: 'application/json',
			data: JSON.stringify({page:page, subcategorie_id : subcategorie_id}),
			success: function (data) {
					...
					if(i == 0 || i == 1 || i == 2 || i == 3){
						...
						$('#product_list').find('.row_1').append('<div class="col-md-3 col-sm-6 c-margin-b-20">'+
																	'<div class="c-content-product-2 c-bg-white c-border">'+
																		...
																		'</div>'+
																		'<div class="c-info" style="height: 60px;">'+
																		...
																		'<div role="group" class="btn-group btn-group-justified">'+
																			'<div role="group" class="btn-group c-border-top"><a href="javascript:;" id="btn_add_whishlist" value="'+_id+'" class="btn btn-sm c-btn-white c-btn-uppercase c-btn-square c-font-grey-3 c-font-white-hover c-bg-red-2-hover c-btn-product">Favorito</a></div>'+
																			'<div role="group" class="btn-group c-border-left c-border-top"><a href="javascript:;" id="btn_add_cart" value="'+_id+'" class="btn btn-sm c-btn-white c-btn-uppercase c-btn-square c-font-grey-3 c-font-white-hover c-bg-red-2-hover c-btn-product">Carrito</a></div>'+
																		'</div>'+
																	'</div>'+
																'</div>')
					}
	...
	$( document ).ready(function() {
		...
		load_products_by_subcategorie_id();
	});
